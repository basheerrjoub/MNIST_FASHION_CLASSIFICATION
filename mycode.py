...
X_test_v = X_test
# print("TYPE: ", types[y_train[1000]])
# print("Feature[180:210]: ", X_train[1000][180:210])
fig, ax = plt.subplots()
img = X_train[1000][:]
img = plt.imshow(img.reshape(28, 28))
plt.close()
# # plt.show()

# Normalizing Features form 0-255 to 0-1
...
def RF_function():
    # GRID SEARCH
    # parameters = {
    #     "n_estimators": [10, 20, 30, 50, 100, 200, 300, 500, 1000],
    #     "max_depth": [1, 5, 10, 20, 25, 30, 40, 50, 60, 70, 80],
    # }
    # random_forrest = RandomForestClassifier()
    # grid = GridSearchCV(estimator=random_forrest, param_grid=parameters, cv=3, n_jobs=-1)
    ...
    #   _____  _      ____ _______   ___   ___
    #  |  __ \| |    / __ \__   __| |__ \ / _ \
    #  | |__) | |   | |  | | | |       ) | | | |
    #  |  ___/| |   | |  | | | |      / /| | | |
    #  | |    | |___| |__| | | |     / /_| |_| |
    #  |_|    |______\____/  |_|    |____|\___/
    ...
    predictions = []
    lines = 0
    with open("wrong.txt", newline="") as file:
        reader = csv.reader(file)
        for row in reader:
            lines += 1
            if lines == 21:
                break
            indices.append(int(row[0].split(": ")[1]))
    ...
    for i, (index, prediction) in enumerate(zip(indices, predictions)):
        ...
        axs[i].set_title(
            " I:{} Actual: {} {}".format(index, types[y_test[index]], prediction),
            fontsize=7,
        )
        axs[i].axis("off")
    plt.show()
